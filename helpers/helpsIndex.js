...
export function createBotonesCategoria(categories, containerCategoria) {
    categories.map(categorie => {
        const button = document.createElement("button");
        button.innerText = categorie;
        button.id = categorie;
        button.classList.add("btnCategories")
        containerCategoria.appendChild(button);
    });
}
export function createMueble(container, mueble) {
...
export function createCategoria(categories, mueblesOficina){
    mueblesOficina.map((mueble) => {
        if (!categories.find(categorie => categorie == mueble.categoria)) categories.push(mueble.categoria);
      })
}